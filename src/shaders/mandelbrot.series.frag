/*
 * Mandelbrot Fragment Shader (Series Approximation + Perturbation)
 * @author    Radim Brnka
 * @project   Synaptory Fractal Traveler
 * @copyright 2025-2026
 * @description
 * High-precision Mandelbrot using Series Approximation to skip iterations,
 * then continues with perturbation for remaining iterations.
 * This reduces accumulated floating-point error at high iteration counts.
 *
 * @license   MIT
 */
...
// Series approximation coefficients texture
uniform sampler2D u_coeffTex;
uniform float u_coeffW;
uniform float u_skipIter;  // Iteration to skip to using series approximation

const int MAX_ITER = __MAX_ITER__;

// --- df helpers (double-float emulation) ---
...
// Sample reference orbit at iteration n
...
// Sample series coefficient A at iteration n
// Coefficients stored as: row 0 = A, row 1 = B
df2 sampleCoeffA(int n){
    float x = (float(n) + 0.5) / u_coeffW;
    vec4 t = texture2D(u_coeffTex, vec2(x, 0.25));  // row 0
    return df2_make(df_make(t.r, t.g), df_make(t.b, t.a));
}

df2 sampleCoeffB(int n){
    float x = (float(n) + 0.5) / u_coeffW;
    vec4 t = texture2D(u_coeffTex, vec2(x, 0.75));  // row 1
    return df2_make(df_make(t.r, t.g), df_make(t.b, t.a));
}
...
void main() {
    ...
    // Series Approximation: skip iterations if enabled
    if (u_skipIter > 0.0) {
        int skipN = int(u_skipIter);

        // Sample coefficients A and B at skip point
        df2 A = sampleCoeffA(skipN);
        df2 B = sampleCoeffB(skipN);

        // dz_N ≈ A_N * dc + B_N * dc²
        df2 dc2 = df2_sqr(dc);
        df2 Adc = df2_mul(A, dc);
        df2 Bdc2 = df2_mul(B, dc2);
        dz = df2_add(Adc, Bdc2);

        startIter = skipN;
    } else {
        dz = df2_make(df_from(0.0), df_from(0.0));
    }
    ...
    // Continue with perturbation iteration from startIter
    for (int n = 0; n < MAX_ITER; n++) {
        if (n < startIter) continue;  // Skip to startIter
